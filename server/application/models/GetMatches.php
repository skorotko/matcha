...
class GetMatches extends Model {
		...
		if (count($allProfilesFromMysql)) {
			...
			if (count($allProfilesFromMysql)) {
				...
				foreach ($allProfilesFromMysql as $key => $value) {
					...
					$allProfilesFromMysql[$key]['newid'] = $counter;
					htmlspecialchars_decode($allProfilesFromMysql[$key]['biography']);
				}
				if (count($allProfilesFromMysql)) {
					print_r(json_encode($allProfilesFromMysql));
					exit;
				}
			}
		}
